...
export const RenderFileList = ({documents,handleDeleteDocument}:{
    documents:Document[],
    handleDeleteDocument:(id:number,url:string, setIsDeleting:React.Dispatch<React.SetStateAction<boolean>>)=>void,
})=>{
    return(
        <>
            {documents.map((document,index) => {
                return(
                    <RenderDocument key={index} id={document.id} name={document.name} url={document.url} type={document.type}  handleDeleteFile={handleDeleteDocument} />
            )})}
        </>
  )}

  const RenderDocument = ({id,name,url,type,handleDeleteFile}: {
    id:number,
    name:string,
    url:string,
    type:string,
    ...
    return(
        <Link
            ...
            >
                ...
                <span className="whitespace-nowrap overflow-hidden text-ellipsis text-sm">{name}</span>
                {!isDeleting ?
                <span
