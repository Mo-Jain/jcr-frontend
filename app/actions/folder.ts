...
export async function createFolder(name:string,parent:"booking"|"car"|"customer"|"profile"){
  const FOLDER_ID = parent === "booking" ? BOOKING_FOLDER_ID : parent === "car" ? CAR_FOLDER_ID : parent === "customer" ? CUSTOMER_FOLDER_ID : PROFILE_FOLDER_ID;
  try{
...
export async function renameFolder(folderId: string, newName: string) {
    try {
      if (!folderId || !newName) throw new Error("Folder ID and new name are required");
      ...
      return { success: true, message: "Folder renamed successfully", folderId, newName };
    } catch (error) {
      console.error("Rename Folder Error:", error);
      return { success: false, error: (error as Error).message };
    }
