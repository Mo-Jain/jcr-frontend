...
const MonthEarnings = () => {
    ...
    async function fetchData() {
      try {
        const res = await axios.get(
          `${BASE_URL}/api/v1/car/thismonth/earnings/all`,
          {
            headers: {
  ...
  return (
    <div className="w-full  relative p-1 rounded-xl shadow-sm bg-white dark:bg-muted z-0 flex flex-col xs:bg-blue-500">
          ...
          <div className="flex flex-col gap-1 scrollbar-hide rounded-lg overflow-y-scroll">
            {earnings.map((earning: Earning) => {
              return (
                ...
                >
                  <div
                    style={{ backgroundColor: earning.colorOfBooking }}
                    className="sm:w-8 z-0 flex-shrink-0 sm:h-8 w-6 h-6 rounded-md"
                  />
                  <div className="flex items-center w-full">
                    <div className="font-semibold text-[#5B4B49] text-xs lg:text-sm dark:text-gray-400">
                      <p className="whitespace-nowrap">
                        {earning.brand} {earning.model}
                      </p>
