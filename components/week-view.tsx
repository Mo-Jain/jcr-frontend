...
export default function WeekView() {
  ...
  return (
    <>
      <div className="grid grid-cols-[auto_1fr_1fr_1fr_1fr_1fr_1fr_1fr] border-b border-border px-4 py-2 h-fit transition-all duration-300 ease-in-out">
          ...
          {getWeekDays(userSelectedDate).map(
            ({ isCurrentDay, today }, index) => {
              ...
              return (
                <div
                  key={index}
                  className="relative border-1 border-black border-r border-border"
                >
                  {getHours.map((hour, i) => {
                    const filteredEvents = getFormatedEvents(
                      events,
                      dayDate.hour(hour.hour()),
                    );
                    return (
                      <div
                        key={i}
