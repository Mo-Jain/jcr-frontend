...
const DateButtons = ({ open }: { open: boolean }) => {
  ...
  return (
    <>
      {isSmallScreen && (
        <div
          className={`${open ? "mt-0 " : " mt-[-70px] opacity-0 "}  duration-300 ease-in-out`}
        >
          <div
            className={`${selectedView != "month" && !open ? "mt-[-334px] opacity-0" : "mt-[-30px] mb-[0]"} my-0 duration-300 ease-in-out`}
          >
            {selectedView != "month" && <SideBarCalendar />}
          </div>
          <div className=" flex mt-1 w-full justify-around p-2 overflow-scroll scrollbar-hide">
            {months.map((month, index) => (
              <div
                key={index}
                className={`p-1 text-sm ${selectedMonth === month ? "bg-primary" : "bg-transparent"} cursor-pointer px-2 border border-border rounded-md`}
