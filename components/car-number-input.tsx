...
const CarNumberPlateInput: React.FC<CarNumberPlateInputProps> = ({ value, onChange }) => {
  const [focused, setFocused] = useState(false)

  const formatValue = useCallback((input: string) => {
    const cleaned = input.replace(/[^A-Za-z0-9]/g, "").toUpperCase()
    const parts = [cleaned.slice(0, 2), cleaned.slice(2, 4), cleaned.slice(4, 6), cleaned.slice(6, 10)]
